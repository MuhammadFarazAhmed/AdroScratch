...
@Singleton
public class PromoApisEncryptionUtils {
    private CLibController controller;
    private static PromoApisEncryptionUtils encryptionUtils;
    private Cipher cipherInstance;
    private IvParameterSpec ivSalt;
    private SecretKeySpec aesSecretKeySpec;

    @Inject
    PromoApisEncryptionUtils(CLibController controller) {
    ...
    private IvParameterSpec getIVSalt() {
        try {
            if (ivSalt == null) {
                ...
                ivSalt = new IvParameterSpec(controller.getPromoAuSaltKey("").getBytes("UTF-8"));
            }
    ...
    private SecretKeySpec getSecretKeySpec() {
        try {
            if (aesSecretKeySpec == null) {
                ...
                aesSecretKeySpec = new SecretKeySpec(controller.getPromoAuSKey("").getBytes("UTF-8"), "AES");
            }
        } catch (UnsupportedEncodingException ignore) {
        }
